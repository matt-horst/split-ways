...
func Reset(ctx context.Context, db *sql.DB, queries *database.Queries) error {
	tx, err := db.Begin()
	if err != nil {
		return fmt.Errorf("couldn't begin transaction: %v\n", err)
	}
	defer tx.Rollback()
	...
	if err := qtx.DeleteAllGroups(ctx); err != nil {
		return fmt.Errorf("couldn't delete all groups: %v\n", err)
	}

	if err := qtx.DeleteAllUsers(ctx); err != nil {
		return fmt.Errorf("couldn't delete all users: %v\n", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("couldn't commit transaction: %v\n", err)
	}
